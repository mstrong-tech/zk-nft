...
function App() {
  ...
  async function mint() {
    ...
    if (
      ...
      try {
        ...
        } else if (err.message.includes("zNFT: ")) {
          const innerErrorMsg = err.message.split("\n")[3];
          var contractError = innerErrorMsg.split("zNFT: ").pop();
  ...
  return (
    <div className="App">
      <Container>
        <Body>
          <Title>Mint NFTs with ZKPs</Title>
          <ToggleContainer>
          ...
          <DivFlex>
            <Button onClick={handleButtonMint}>Mint (Goerli)</Button>
          </DivFlex>
