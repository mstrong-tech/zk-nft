...
contract MintZKNft is ERC721, Ownable {
    ...
    function mintPlonk(bytes memory proof, uint256[] memory pubSignals)
        ...
        payable
    {
        require(totalSupply + AMOUNT <= SALE_MAX, "zNFT: Max Minted");
        require(
            plonkVerifier.verifyProof(proof, pubSignals),
            "zNFT: Contract Mint Not Allowed"
        );
    ...
    {
        require(_exists(tokenId), "zNFT: Cannot query non-existent token");
